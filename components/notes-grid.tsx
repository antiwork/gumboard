...
import {
  ...
  convertPositionedNotesToColumns,
} from "@/lib/utils";
...
export function NotesGrid({
  ...
  const masonryClasses = useMemo(
    () => cn("columns-1 gap-4 sm:columns-2", gridClassName),
    [gridClassName]
  );
  ...
  const containerClasses = useMemo(
    () => (isGrid ? gridClasses : masonryClasses),
    [isGrid, gridClasses, masonryClasses]
  );
          ...
          {columns.map((columnNotes, columnIndex) => (
            <div key={columnIndex} className="flex-1 flex flex-col gap-4">
              <AnimatePresence mode="popLayout">
                {columnNotes.map(
                  (note: Note & { x: number; y: number; width: number; height: number }) => (
                    <motion.div
                      key={note.id}
