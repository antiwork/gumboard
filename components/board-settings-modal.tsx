...
export function useBoardSettingsModal() {
  const openBoardSettings = (board: {
    ...
    window.dispatchEvent(
      new CustomEvent("openBoardSettings", {
        detail: board,
      })
    );
  };

  return { openBoardSettings };
}

export const BoardSettingsModal = ({ redirectAfterDelete, onSuccess }: BoardSettingsModalProps) => {
  ...
  useEffect(() => {
    const handleOpenBoardSettings = (event: CustomEvent) => {
      openBoardSettings(event.detail);
    };

    window.addEventListener("openBoardSettings", handleOpenBoardSettings as EventListener);

    return () => {
      window.removeEventListener("openBoardSettings", handleOpenBoardSettings as EventListener);
    };
  }, []);
