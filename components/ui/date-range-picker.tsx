...
function DateRangePicker({
  ...
  // State for the individual calendar popovers
  ...
  const handleStartCalendarSelect = (date: Date | undefined) => {
      ...
      setIsStartPopoverOpen(false); // Close the start date popover
  };

  const handleEndCalendarSelect = (date: Date | undefined) => {
      if (date && internalStartDate && isAfter(internalStartDate, date)) {
          return;
      }
      setInternalEndDate(date || null);
      onDateRangeChange?.(internalStartDate, date || null);
      setIsEndPopoverOpen(false); // Close the end date popover
  };
  ...
  return (
    <div className={cn("relative", className)} ref={dropdownRef}>
      <Popover open={isFilterDropdownOpen} onOpenChange={setIsFilterDropdownOpen}>
        ...
        <PopoverContent className="w-80 bg-white dark:bg-zinc-900 rounded-md shadow-lg border border-zinc-200 dark:border-zinc-800 z-50 p-4">
              ...
              <div>
                ...
                <Popover open={isStartPopoverOpen} onOpenChange={setIsStartPopoverOpen}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      {internalStartDate ? format(internalStartDate, "LLL dd, y") : "Pick a start date"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0 text-muted-foreground dark:text-zinc-200">
                    <Calendar
                      ...
                      classNames={{day: "hover:bg-sky-500 hover:text-white transition-colors duration-200 rounded"}}
                    />
                  </PopoverContent>
                </Popover>
              </div>
                ...
                <Popover open={isEndPopoverOpen} onOpenChange={setIsEndPopoverOpen}>
                  ...
                  <PopoverContent className="w-auto p-0 text-muted-foreground dark:text-zinc-200">
                    <Calendar
                      ...
                      disabled={(date) => isAfter(internalStartDate || new Date(), date)}
                      classNames={{day: "hover:bg-sky-500 hover:text-white transition-colors rounded"}}
                    />
                  </PopoverContent>
                </Popover>
              </div>
