...
export class NLPService {
  ...
  private parseWithRules(text: string): NLPIntent {
    ...
    if (addMatch && addMatch[2]) { // Fixed: Check array index exists
      return {
        action: 'add',
        confidence: 0.85,
    ...
    // Complete intentions
    const completeNumberMatch = lowerText.match(/\b(complete|done|finished?)\s+(?:task\s+)?(\d+)\b/);
    if (completeNumberMatch && completeNumberMatch[2]) { // Fixed: Check array index exists
      return {
        action: 'complete',
        confidence: 0.9,
    ...
    const completeTextMatch = lowerText.match(/\b(mark|complete|finish)\b.*\b(.*?)\b.*\b(done|complete)\b/);
    if (completeTextMatch && completeTextMatch[2]) { // Fixed: Check array index exists
      return {
        action: 'complete',
        confidence: 0.7,
    ...
    if (removeNumberMatch && removeNumberMatch[2]) { // Fixed: Check array index exists
      return {
        action: 'remove',
        confidence: 0.9,
    ...
    // Edit intentions
    const editMatch = lowerText.match(/\b(change|edit|update)\s+(?:task\s+)?(\d+)\s+(?:to\s+)?(.+)/);
    if (editMatch && editMatch[2] && editMatch[3]) { // Fixed: Check array indices exist
      return {
        action: 'edit',
        confidence: 0.85,
        entities: { 
