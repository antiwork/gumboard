...
describe("formatNoteForDiscord", () => {
  ...
  it("should handle special characters in content", () => {
    ...
    expect(result).toBe(
      ":heavy_plus_sign: Note with @mentions and #hashtags! by Jane Smith in Special Board"
    );
  });
  ...
  it("should handle Unicode characters", () => {
    ...
    expect(result).toBe(
      ":heavy_plus_sign: Unicode note 🚀 with émojis by María José in Unicode Board"
    );
  });
...
describe("formatTodoForDiscord", () => {
  ...
  it("should handle special characters in todo content", () => {
    ...
    expect(result).toBe(
      ":white_check_mark: Fix bug #123 & test @reviewer by Dev Team in Bug Board"
    );
  });
});
