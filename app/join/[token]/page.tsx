...
async function joinOrganization(token: string) {
  ...
  const inviteResult = await validateInvite(token);
  if (!inviteResult.success || !inviteResult.invite) return; 
  ...
  await db.user.update({
    ...
    data: { organizationId: inviteResult.invite.organizationId },
  });
...
async function autoCreateAccountAndJoin(token: string, formData: FormData) {
  ...
  try {
    ...
    if (!invite.success || !invite.invite) throw new Error("Invalid invitation");
    ...
    if (!user) {
      ...
      if (user.organizationId !== invite.invite.organizationId) {
        throw new Error("Already in another organization");
      }
    } else {
      user = await db.user.update({
        where: { id: user.id },
        data: { organizationId: invite.invite.organizationId },
      });
    }
    ...
    await db.session.create({
      data: { sessionToken, userId: user.id, expires: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) },
    });
...
export default async function JoinPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!inviteResult.success) {
    return <ErrorCard title={inviteResult.error!.title} description={inviteResult.error!.description} />;
  }
  ...
  const user = await db.user.findUnique({ where: { id: session.user.id }, include: { organization: true } });
  ...
  if (user?.organizationId) {
    return <ErrorCard title="Already in Organization" description={`You are already a member of ${user.organization?.name}. You can only be a member of one organization at a time.`} />;
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 dark:bg-zinc-950 p-4">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center space-y-3">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Join {invite.organization.name}</h1>
          <p className="text-lg text-slate-600 dark:text-slate-400">
            You've been invited to join{" "}
            <span className="font-semibold text-slate-800 dark:text-slate-200">{invite.organization.name}</span> on
            Gumboard
          </p>
        </div>
        <Card className="border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950">
          <CardHeader className="text-center pb-6">
            <div className="mx-auto border border-slate-200 dark:border-zinc-800 mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-slate-200 dark:bg-zinc-800">
              <span className="text-2xl font-bold text-slate-900 dark:text-slate-100">{invite.organization.name.charAt(0).toUpperCase()}</span>
            </div>
...
function JoinConfirmationCard({ invite, token }: { invite: NonNullable<ValidationResult['invite']>, token: string }) {
        ...
        <Card className="border-2">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-green-500 to-blue-600">
              <span className="text-2xl font-bold text-white">{invite.organization.name.charAt(0).toUpperCase()}</span>
            </div>
          ...
          <CardContent className="space-y-4">
            <div className="text-center space-y-2">
              <p className="text-sm text-muted-foreground">Created by: {invite.user.name || invite.user.email}</p>
              <p className="text-sm text-muted-foreground">{usageInfo}</p>
              {invite.expiresAt && <p className="text-sm text-muted-foreground">Expires: {invite.expiresAt.toLocaleDateString()}</p>}
            </div>
            <form action={joinOrganization.bind(null, token)}>
              <Button type="submit" className="w-full bg-green-600 hover:bg-green-700">Join {invite.organization.name}</Button>
            </form>
          </CardContent>
        </Card>
      </div>
