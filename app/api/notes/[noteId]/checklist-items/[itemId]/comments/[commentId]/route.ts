...
export async function DELETE(
  request: NextRequest,
  { params }: { params: Promise<{ noteId: string; itemId: string; commentId: string }> }
) {
  try {
    ...
    const { noteId, itemId, commentId } = await params
    ...
    if (comment.noteId !== noteId || comment.checklistItemId !== itemId) {
      return NextResponse.json({ error: "Comment does not belong to this checklist item" }, { status: 400 })
    }
    ...
    if (comment.createdBy !== session.user.id && !user.isAdmin) {
      return NextResponse.json({ error: "Only the comment author or admin can delete this comment" }, { status: 403 })
    }

    // Verify the checklist item exists in the note
    const checklistItems = comment.note.checklistItems as any[];
    const itemExists = checklistItems?.find((item: any) => item.id === itemId);
